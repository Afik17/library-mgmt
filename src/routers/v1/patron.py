...
def search_patrons(
    ...
    return library.search_patrons(criteria=criteria)
...
def get_patron_borrows(
    ...
    return library.get_patron_borrows(patron_id=patron_id, borrow_status=borrow_status)


@router.get("/{patron_id}/borrows/fines", response_model=BorrowFine)
def calculate_patron_overdues_fines(
    patron_id: str,
    library: Annotated[Library, Depends(get_library)],
) -> float:
    total_fines = library.calculate_patron_overdues_fines(patron_id=patron_id)
    return BorrowFine(total_fines=total_fines)


@router.post("", response_model=PatronResponse, status_code=status.HTTP_201_CREATED)
def register_new_patron(
    patron: PatronCreate, library: Annotated[Library, Depends(get_library)]
) -> PatronResponse:
    return library.register_new_patron(patron=patron)
...
def update_patron(
    ...
    return library.update_patron(patron_id=patron_id, patron=patron)
